...
namespace WPF_demo.View.UserControls {
	public partial class BmiChart : UserControl {
		...
		public BmiChart() {
			...
			// set the chart series initially (default to male data)
		...
		private void ApplyVmToChart(ViewModel.BmiChartViewModel vm) {
			// pick male/female baseline series based on proband gender (default to male if unknown)
		...
		/// <summary>
		/// Build the percentile series set for the requested gender.
		/// Defaults to using the male (original) arrays; female arrays are provided
		/// here as duplicates — replace with correct female percentiles as needed.
		/// </summary>
		private ISeries[] BuildSeries(bool isMale) {
			// choose arrays depending on gender
